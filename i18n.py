...
MESSAGES: dict[str, dict[str, str]] = {
    "zh-Hant": {
        ...
        "form.fineprint": "粵語／英文用 OpenAI（Whisper＋GPT）；简体用智譜。請在專案 .env 填 OPENAI_API_KEY 同 ZHIPUAI_API_KEY。淨係貼腳本、冇金鑰時可用本地規則。請淨係上載你有權用嘅內容。",
    ...
    "zh-Hans": {
        ...
        "form.fineprint": "粤语／英文用 OpenAI（Whisper＋GPT）；简体用智谱。请在项目 .env 填写 OPENAI_API_KEY 与 ZHIPUAI_API_KEY。仅贴脚本且无密钥时可用本地规则。请只上传你有权使用的内容。",
    ...
    "en": {
        ...
        "form.fineprint": "Cantonese & English use OpenAI (Whisper + GPT); Simplified Chinese uses Zhipu. Put OPENAI_API_KEY and ZHIPUAI_API_KEY in .env. Script-only works with local rules when no key is set. Upload only content you have rights to use.",
